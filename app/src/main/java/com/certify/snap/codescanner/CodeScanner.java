...
import com.certify.snap.codescanner.CodeScannerException;
import com.certify.snap.codescanner.CodeScannerView;
import com.certify.snap.codescanner.DecodeCallback;
import com.certify.snap.codescanner.DecodeTask;
import com.certify.snap.codescanner.Decoder;
import com.certify.snap.codescanner.DecoderWrapper;
import com.certify.snap.codescanner.ErrorCallback;
import com.certify.snap.codescanner.Point;
import com.certify.snap.codescanner.Rect;
import com.certify.snap.codescanner.ScanMode;
import com.certify.snap.codescanner.Utils;
...
public final class CodeScanner {
    ...
    private static final com.certify.snap.codescanner.ScanMode DEFAULT_SCAN_MODE = com.certify.snap.codescanner.ScanMode.SINGLE;
    ...
    private volatile com.certify.snap.codescanner.ScanMode mScanMode = DEFAULT_SCAN_MODE;
     ...
     */
    @NonNull
    public com.certify.snap.codescanner.ScanMode getScanMode() {
        return mScanMode;
    }

    /**
     * Scan mode, {@link com.certify.snap.codescanner.ScanMode#SINGLE} by default
     *
     * @see com.certify.snap.codescanner.ScanMode
     */
    public void setScanMode(@NonNull final com.certify.snap.codescanner.ScanMode scanMode) {
            ...
            if (zoom != mZoom) {
                ...
                if (mInitialized) {
                    ...
                    if (decoderWrapper != null) {
                        ...
                        com.certify.snap.codescanner.Utils.setZoom(parameters, zoom);
    ...
    void performTouchFocus(final com.certify.snap.codescanner.Rect viewFocusArea) {
        synchronized (mInitializeLock) {
            if (mInitialized && mPreviewActive && !mTouchFocusing) {
                try {
                    ...
                    if (mPreviewActive && decoderWrapper != null &&
                            decoderWrapper.isAutoFocusSupported()) {
                        final com.certify.snap.codescanner.Point imageSize = decoderWrapper.getImageSize();
                        ...
                        final com.certify.snap.codescanner.Rect imageArea =
                                com.certify.snap.codescanner.Utils.getImageFrameRect(imageWidth, imageHeight, viewFocusArea,
                                        decoderWrapper.getPreviewSize(),
                                        decoderWrapper.getViewSize());
                        final Camera camera = decoderWrapper.getCamera();
                        camera.cancelAutoFocus();
                        final Parameters parameters = camera.getParameters();
                        com.certify.snap.codescanner.Utils.configureFocusArea(parameters, imageArea, imageWidth, imageHeight,
                                orientation);
                        com.certify.snap.codescanner.Utils.configureFocusModeForTouch(parameters);
                        camera.setParameters(parameters);
        ...
        try {
            ...
            if (decoderWrapper != null) {
                ...
                if (decoderWrapper.isAutoFocusSupported() && mAutoFocusEnabled) {
                    final com.certify.snap.codescanner.Rect frameRect = mScannerView.getFrameRect();
                    if (frameRect != null) {
                        final Parameters parameters = camera.getParameters();
                        com.certify.snap.codescanner.Utils.configureDefaultFocusArea(parameters, decoderWrapper, frameRect);
                        camera.setParameters(parameters);
                    }
    ...
    private void stopPreviewInternal(final boolean internal) {
        try {
            ...
            if (decoderWrapper != null) {
                ...
                final Parameters parameters = camera.getParameters();
                if (!internal && decoderWrapper.isFlashSupported() && mFlashEnabled) {
                    com.certify.snap.codescanner.Utils.setFlashMode(parameters, Parameters.FLASH_MODE_OFF);
                }
                camera.setParameters(parameters);
    ...
    private void setFlashEnabledInternal(final boolean flashEnabled) {
        try {
            ...
            if (decoderWrapper != null) {
                ...
                if (flashEnabled) {
                    com.certify.snap.codescanner.Utils.setFlashMode(parameters, Parameters.FLASH_MODE_TORCH);
                } else {
                    com.certify.snap.codescanner.Utils.setFlashMode(parameters, Parameters.FLASH_MODE_OFF);
                }
                camera.setParameters(parameters);
    ...
    private void setAutoFocusEnabledInternal(final boolean autoFocusEnabled) {
        try {
            ...
            if (decoderWrapper != null) {
                ...
                if (autoFocusEnabled) {
                    com.certify.snap.codescanner.Utils.setAutoFocusMode(parameters, autoFocusMode);
                } else {
                    com.certify.snap.codescanner.Utils.disableAutoFocus(parameters);
                }
                if (autoFocusEnabled) {
                    final com.certify.snap.codescanner.Rect frameRect = mScannerView.getFrameRect();
                    if (frameRect != null) {
    ...
    private final class PreviewCallback implements Camera.PreviewCallback {
        ...
        public void onPreviewFrame(final byte[] data, final Camera camera) {
            if (!mInitialized || mStoppingPreview || mScanMode == com.certify.snap.codescanner.ScanMode.PREVIEW ||
                    data == null) {
                return;
            }
            final DecoderWrapper decoderWrapper = mDecoderWrapper;
            ...
            final com.certify.snap.codescanner.Decoder decoder = decoderWrapper.getDecoder();
            if (decoder.getState() != com.certify.snap.codescanner.Decoder.State.IDLE) {
                return;
            }
            final com.certify.snap.codescanner.Rect frameRect = mScannerView.getFrameRect();
            ...
            decoder.decode(new com.certify.snap.codescanner.DecodeTask(data, decoderWrapper.getImageSize(),
    ...
    private final class DecoderStateListener implements com.certify.snap.codescanner.Decoder.StateListener {
        @Override
        public boolean onStateChanged(@NonNull final com.certify.snap.codescanner.Decoder.State state) {
            if (state == com.certify.snap.codescanner.Decoder.State.DECODED) {
                final com.certify.snap.codescanner.ScanMode scanMode = mScanMode;
                if (scanMode == com.certify.snap.codescanner.ScanMode.PREVIEW) {
                    return false;
    ...
    private final class InitializationThread extends Thread {
        ...
        private void initialize() {
            ...
            final int orientation = com.certify.snap.codescanner.Utils.getDisplayOrientation(mContext, cameraInfo);
            final boolean portrait = com.certify.snap.codescanner.Utils.isPortrait(orientation);
            final com.certify.snap.codescanner.Point imageSize =
                    com.certify.snap.codescanner.Utils.findSuitableImageSize(parameters, portrait ? mHeight : mWidth,
            ...
            final com.certify.snap.codescanner.Point previewSize = com.certify.snap.codescanner.Utils.getPreviewSize(portrait ? imageHeight : imageWidth,
            ...
            final com.certify.snap.codescanner.Point viewSize = new com.certify.snap.codescanner.Point(mWidth, mHeight);
            if (autoFocusSupported && mAutoFocusEnabled) {
                com.certify.snap.codescanner.Utils.setAutoFocusMode(parameters, mAutoFocusMode);
                final com.certify.snap.codescanner.Rect frameRect = mScannerView.getFrameRect();
                if (frameRect != null) {
                    com.certify.snap.codescanner.Utils.configureDefaultFocusArea(parameters, frameRect, previewSize, viewSize,
            ...
            com.certify.snap.codescanner.Utils.configureFpsRange(parameters);
            com.certify.snap.codescanner.Utils.configureSceneMode(parameters);
            com.certify.snap.codescanner.Utils.configureVideoStabilization(parameters);
            camera.setParameters(parameters);
            camera.setDisplayOrientation(orientation);
            synchronized (mInitializeLock) {
                final com.certify.snap.codescanner.Decoder decoder =
                        new com.certify.snap.codescanner.Decoder(mDecoderStateListener, mFormats, mDecodeCallback);
                mDecoderWrapper =
    ...
    private final class FinishInitializationTask implements Runnable {
        ...
        private FinishInitializationTask(@NonNull final com.certify.snap.codescanner.Point previewSize) {
