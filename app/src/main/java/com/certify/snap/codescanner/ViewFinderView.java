...
final class ViewFinderView extends View {
    ...
    private com.certify.snap.codescanner.Rect mFrameRect;
    ...
    protected void onDraw(@NonNull final Canvas canvas) {
        final com.certify.snap.codescanner.Rect frame = mFrameRect;
        if (frame == null) {
            return;
        }
    ...
    private void invalidateFrameRect(final int width, final int height) {
        if (width > 0 && height > 0) {
            ...
            mFrameRect =
                    new com.certify.snap.codescanner.Rect(frameLeft, frameTop, frameLeft + frameWidth, frameTop + frameHeight);
        }
    }
}
