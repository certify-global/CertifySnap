...
public class FaceServer {
     ...
     * @return 是否注册成功
     */
    public boolean registerBgr24Test(Context context, byte[] bgr24, int width, int height) {
        synchronized (this) {
            if (faceEngine == null || context == null || bgr24 == null || width % 4 != 0 || bgr24.length != width * height * 3) {
            ...
            if (code == ErrorInfo.MOK && faceInfoList.size() > 0) {
                ...
                try {
                    //保存注册结果（注册图、特征数据）
                    if (code == ErrorInfo.MOK) {
                        Log.e(TAG, "registerBgr24: extract face feature true");
                        return true;
                    } else {
                        Log.e(TAG, "registerBgr24: extract face feature failed, code is " + code);
                        return false;
    ...
    public void deleteImgFeature(Context context,String name){
        try {
            ...
            //特征存储的文件夹
            File featureDir = new File(ROOT_PATH + File.separator + SAVE_FEATURE_DIR);
            if (!featureDir.exists() && !featureDir.mkdirs()) Log.e(TAG, "not have feature directory");

            //图片存储的文件夹
            File imgDir = new File(ROOT_PATH + File.separator + SAVE_IMG_DIR);
            if (!imgDir.exists() && !imgDir.mkdirs()) Log.e(TAG, "not have image directory");


            String image = imgDir + File.separator + name + FaceServer.IMG_SUFFIX;
            String feature = featureDir + File.separator + name;
